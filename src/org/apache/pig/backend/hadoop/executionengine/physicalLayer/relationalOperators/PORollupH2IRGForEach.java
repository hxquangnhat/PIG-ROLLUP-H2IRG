...
/**
 * This class provides a new ForEach physical operator
 * to handle the ROLLUP with hybrid IRG when the Rollup
 * Optimizer is activated.
 * This class contains almost the same as POForEach class
 * excepts some functions for the Hybrid IRG stuffs.
 */
//We intentionally skip type checking in backend for performance reasons
@SuppressWarnings("unchecked")
public class PORollupH2IRGForEach extends PhysicalOperator {
    ...
    //The first tuple that stores the value of the previous Rollup Dimension for the first IRG
    protected Tuple prevRollupDimension = null;

    //The second tuple that stores the value of the previous Rollup Dimension for the second IRG
    ...
    protected Integer condition_position = null;
    
    private Integer rollup_size = 0;
    
    private Integer rollup_start_position = 0;
    
    private Integer output_index[] = null;
    ...
    /**
     * We create a template for output the fields in a tuple
     * in case the rollup operation has been moved to the end
     * of the operation list
     * @param len
     */
    public void output_index_Init(int len) {
    	output_index = new Integer[len];
        for(int i = 0; i < len - this.rollup_start_position; i++)
        	if(i<this.rollup_start_position)
        		output_index[i] = i;
        	else
        		output_index[i] = i + rollup_size;
        
        int count  = this.rollup_start_position;
        
        for(int i = len - this.rollup_position; i < len; i++)
    ...
    /**
     * Set the original index of the first field of Rollup operation
     * In case the rollup operation has been moved to the end of the operation list
     * @param ru_start_pos
     */
    ...
    public void setRollupSize(Integer ru_len) {
    	this.rollup_size = ru_len;
    }
    
    public Integer getRollupSize() {
    	return this.rollup_size;
    }
    
    /**
     * Set the updated index of the first field of Rollup operation
     * and also update the new pivot position due to the change of the rollup operation position
     * In case the rollup operation has been moved to the end of the operation list
     * @param ru_pos
     */
    public void setRollupPosition(Integer ru_pos) {
    	...
    	if(onlyIRG != true) {
    		...
    		condition_position = pivot;
    	}
    	else
    		condition_position = pivot + rollup_position;
    }
    ...
    public PORollupH2IRGForEach(OperatorKey k, int rp, List<PhysicalPlan> inp, List<Boolean>  isToBeFlattened){
        super(k, rp);
    ...
    public PORollupH2IRGForEach(OperatorKey operatorKey, int requestedParallelism, List<PhysicalPlan> innerPlans, List<Boolean> flattenList, Schema schema) {
    ...
    public void visit(PhyPlanVisitor v) throws VisitorException {
        v.visitPORollupH2IRGForEach(this);
    }

    @Override
    public String name() {
        return getAliasString() + "New Rollup H2IRG For Each" + " (" + getFlatStr() + ")" + "["
    ...
    protected void computeRollup() throws ExecException {
        ...
        if (rollup_position!=0)
        	index = rollup_position - 1;
        
        if (prevRollupDimension.get(len - 1) != null) {
        	for (int i = 0; i < rollup_position; i++)
        		if (DataType.compare(currentRollupDimension.get(i), prevRollupDimension.get(i)) != 0) {
        			i_multi = rollup_position - 1;
        			break;
        		}
        	
        	for (int i = index; i < len - 1; i++) {
	            if (DataType.compare(currentRollupDimension.get(i), prevRollupDimension.get(i)) != 0 || (rollup_position!=0 && finIRG1 == true) || i_multi!=-1) {
	            	
	            	//find the maximum value of the first index that differs the 
	            	//currentRollupDimension and prevRollupDimension and the pivot
	            	int i_temp = Math.max(i, i_multi);
	            	int x = Math.max(i_temp, pivot);

	            	//create the missing tuples for rollup operation in the first IRG
    ...
    protected void computeRollup2() throws ExecException {
        ...
        for (int i = index; i < pivot; i++) {
            if (DataType.compare(currentRollupDimension.get(i), prevRollupDimension2.get(i)) != 0 || finIRG2 == true || i_multi!=-1) 
            {
            	int x = Math.max(i_multi, i);
            	//create the missing tuples for rollup operation in the second IRG
    ...
    /**
     * Call the final aggregation for the IRGs
    ...
    public Result getNextTuple() throws ExecException {
        try {
            ...
            //Check if 
            if(rollup_position!=0 && modified_pivot == false && pivot == 0) {
            	modified_pivot = true;
            }
            ...
            while (true) {
                ...
                if (inpTuple.getType(0) == DataType.TUPLE) {
                    ...
                    //The special record which has size larger than the default one
                    //to mark that we went through the last record, compute the final
                    ...
                    if(checkLast)
                    {
            			...
            			if(tmp!=null) {
            				...
            				res = new Result();
            				res.result = returnRes[returnRes.length - 1].result; //change the NULL tuple to one of the remaining tuples
            				res.returnStatus = POStatus.STATUS_OK;
            				returnRes[returnRes.length - 1] = null;
                    ...
                    if(output_index == null && rollup_position!=0 )
                    	this.output_index_Init(len);
                ...
                //rollup operation to the end of the operation list) pivot position
                //of the tuple is null or not. If it is not null, we compute the
                //rollup using the first rollup, else, we compute the it using the second IRG
                if(currentRollupDimension.get(condition_position)!=null) {
                	secondPass = false;
                	if (prevRollupDimension != null) {
                		computeRollup();
                	} else {
                ...
                if (isAccumulative()) {
                	...
                	if(!secondPass)
                		returnRes[0] = processPlan(currentRollupDimension.size() - 1);
                	//else, we process the rollup and store it in the returnRes2
                	//if the pivot is zero, it's IRG, else, we process at the (pivot - 1)
                	//because the pivot position user specified is always larger than the 
                ...
                //We return the result that we stored in returnRes or returnRes2
            ...
            throw new ExecException("Error while executing RollupH2IRGForEach at " + this.getOriginalLocations(), e);
        }
    }
        ...
        if(its == null) {
            ...
            for(int i = 0; i < noItems; ++i) {
                ...
                case DataType.CHARARRAY :
                    ...
                    //We stores the payloads that we want to compute the rollup in tmpResult
...
//                Object input = null;
    ...
    protected Tuple createTuple(Object[] data) throws ExecException {
        ...
        for(int i = 0; i < data.length; ++i) {
            ...
            if ((isToBeFlattenedArray[i] || rollup_position!=0) && in instanceof Tuple) {
                Tuple t = (Tuple)in;
                int size = t.size();
    ...
    public PORollupH2IRGForEach clone() throws CloneNotSupportedException {
        ...
        PORollupH2IRGForEach clone = new PORollupH2IRGForEach(new OperatorKey(mKey.scope,
