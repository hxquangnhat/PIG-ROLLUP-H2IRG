...
public class RollupHIIPartitioner extends HashPartitioner<PigNullableWritable, Writable> implements Configurable {
    ...
    public void setConf(Configuration conf) {
        //Get the pivot
        pivot = conf.getInt(PigConfiguration.PIG_HII_ROLLUP_PIVOT, -1);
        //Get the index of the first field involves in ROLLUP
        rollupFieldIndex = conf.getInt(PigConfiguration.PIG_HII_ROLLUP_FIELD_INDEX, 0);
        ...
        rollupOldFieldIndex = conf.getInt(PigConfiguration.PIG_HII_ROLLUP_OLD_FIELD_INDEX, 0);
        //Get the size of total fields that involve in CUBE clause
        length = conf.getInt(PigConfiguration.PIG_HII_NUMBER_TOTAL_FIELD, 0);
